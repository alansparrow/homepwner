...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        _borderImageView = [[UIImageView alloc]
                            initWithImage:[[UIImage imageNamed:@"border.png"]
                            resizableImageWithCapInsets:UIEdgeInsetsMake(CAP_INSET,
                                                                         CAP_INSET,
                                                                         CAP_INSET,
                                                                         CAP_INSET)]];
...
{
    return UIEdgeInsetsMake(CONTENT_INSET,
                            CONTENT_INSET,
                            CONTENT_INSET,
                            CONTENT_INSET);
}
...
- (void)layoutSubviews
{
    ...
    switch (self.arrowDirection) {
        case UIPopoverArrowDirectionUp:
            ...
            _coordinate = ((self.frame.size.width / 2) +
                           self.arrowOffset) - (ARROW_BASE/2);
